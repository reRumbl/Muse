...
from src.auth.exceptions import (
    UsernameAlreadyRegisteredException, IncorrectUsernameOrPasswordException,
...
class AuthService:
    ...
    async def register(self, data: UserRegister) -> User:
        user = await UserModel.find_by_username(self.session, data.username)
        if user:
    ...
    async def login(self, data: UserLogin) -> TokenPairResponse:
        user = await UserModel.authenticate(self.session, data.username, data.password.get_secret_value())
        if not user:
